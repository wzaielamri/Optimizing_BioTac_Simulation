...
Best_config = {
    ...
    'colsample_bytree': 0.7449, 'colsample_bylevel': 0.9264, 'colsample_bynode':0.9802, 
    } 
...
def measure_inference_usage(models, test_inputs):
    inference=[]
    for _ in range(100):
        time_elapsed=0
        rand_ind=np.random.randint(0, len(test_inputs)-1)
        test_input=test_inputs[0+rand_ind:1+rand_ind]
        for model in models:
            start = time.time()
            _ = model.predict(test_input)
            end = time.time()
            time_elapsed += (end - start)
        time_elapsed_ms = (time_elapsed/21)* 1000 # ms
        inference.append(time_elapsed_ms)
...
for train_data_in, train_data_out, train_data_in_scaled, train_data_out_scaled, test_data_in, test_data_out, test_data_in_scaled, test_data_out_scaled, mean_out, std_out  in zip(train_data_in_folds, train_data_out_folds, train_data_in_scaled_folds, train_data_out_scaled_folds, test_data_in_folds, test_data_out_folds, test_data_in_scaled_folds, test_data_out_scaled_folds, mean_train_out_folds, std_train_out_folds):
    ...
    inference_mean, inference_std = measure_inference_usage(multi_reg, test_data_in_scaled,)
    inference_list.append(inference_mean)
    inference_std_list.append(inference_std)
    
    fold_ind+=1
    #break 


wandb.log({"inference_time": np.mean(inference_list)})
wandb.log({"inference_time_std": np.mean(inference_std_list)})
